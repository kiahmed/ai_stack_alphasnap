...
if gcloud scheduler jobs describe "$SCHEDULER_NAME" --location="$LOCATION" > /dev/null 2>&1; then
    ...
    OUTPUT=$(gcloud scheduler jobs update http "$SCHEDULER_NAME" \
        ...
        --message-body='{"class_method": "stream_query", "input": {"user_id": "cron_scheduler", "message": "Execute the daily market sweep. Run the CIO to gather findings from scouts, log them, and print the tabular report."}}' \
